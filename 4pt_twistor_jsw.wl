...
(*Lorentz contraction with itself*)
mp2[v1_]:=mp[v1,v1];
(*epsilon*)
epsilonup=({
 {0, 1},
...
(*from given two paired spinors to the vector matrix*)
vM[\[Lambda]_,\[Lambda]t_]:=Transpose[{\[Lambda]}].{\[Lambda]t};
(*further obtain this vector itself*)
...
(*from two arbitary spinors pair to a new vector*)
...
(*computing lambda tilde*)
lambda={\[Lambda][1],\[Lambda][2],\[Lambda][3],\[Lambda][4]};
mu={\[Mu][1],\[Mu][2],\[Mu][3],\[Mu][4]};
lambdat=Table[0,{j,1,4}];
For[i=1,i<=4,i++,
ip=i+1;
If[ip>4,ip-=4];
im=i-1;
...
(*lambda tilde*)
{\[Lambda]t[1],\[Lambda]t[2],\[Lambda]t[3],\[Lambda]t[4]}=lambdat//Simplify
(*extract 4 particles' momenta*)
P=Table[v[\[Lambda][j],\[Lambda]t[j]],{j,1,4}]//Simplify
(*check momentum conservation*)
Sum[v[\[Lambda][j],\[Lambda]t[j]],{j,1,4}]//Simplify
(*spinor product*)
AA[i_,j_]:=aa[\[Lambda][i],\[Lambda][j]];
BB[i_,j_]:=bb[\[Lambda]t[i],\[Lambda]t[j]];


(*construct polarization vector from a given reference momentum*)
varepsilonP[\[Lambda]1_,\[Lambda]1t_,\[Lambda]1k_]:=Module[{\[Lambda]=\[Lambda]1,\[Lambda]t=\[Lambda]1t,\[Lambda]k=\[Lambda]1k,result},
result=Sqrt[2] newv[\[Lambda]k,\[Lambda]t]/aa[\[Lambda]k,\[Lambda]];
Return[result];
];
varepsilonM[\[Lambda]1_,\[Lambda]1t_,\[Lambda]1kt_]:=Module[{\[Lambda]=\[Lambda]1,\[Lambda]t=\[Lambda]1t,\[Lambda]kt=\[Lambda]1kt,result},
result=Sqrt[2] newv[\[Lambda],\[Lambda]kt]/bb[\[Lambda]t,\[Lambda]kt];
Return[result];
];


(*Table[AA[i,j],{i,1,4},{j,1,4}]//FullSimplify//MatrixForm;
...
Gram4=Table[mp[P[[i]],P[[j]]],{i,1,4},{j,1,4}]//FullSimplify;
S[i_,j_]:=2 Gram4[[i,j]];
ss=Solve[{s==2 Gram4[[1,2]],t==2 Gram4[[1,4]]},{x1,x2}]//Simplify;
